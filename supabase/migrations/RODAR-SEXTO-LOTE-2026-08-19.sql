-- ===========================================================================
-- COMMANDER — SEXTO LOTE, 19/08/2026
-- AVISAR O PRESTADOR, E A CONVERSA DENTRO DO APP
--
-- ***** ESTE É O ÚNICO LOTE QUE TRAVA O DEPLOY. *****
--
-- Os cinco lotes anteriores endureciam regra sobre tabela que já existia — o
-- app rodava com ou sem eles. Este cria tabela NOVA, e o código da onda 99 já
-- está no repositório lendo essas tabelas. Subir o app antes de rodar isto
-- quebra a tela de Marketplace e a caixa de mensagens. Rode primeiro, publique
-- depois.
--
-- Cole INTEIRO no SQL Editor do Supabase. A conferência sai no fim.
--
-- SÃO DUAS TRANSAÇÕES, NÃO UMA, e isso é de propósito: cada migration abre e
-- fecha a sua. Se a segunda falhar, a primeira já está gravada.
--
-- A ORDEM NÃO É OBRIGATÓRIA — as duas são independentes — mas está certa como
-- está: o aviso é o que traz gente para o Marketplace, e a conversa é o que
-- acontece depois que ela chega.
--
-- ---------------------------------------------------------------------------
-- O QUE CADA UMA FAZ
-- ---------------------------------------------------------------------------
-- 089  `avisos_demanda` — a memória de quem já foi avisado de qual pedido.
--      Existe por UM motivo: sem ela o mesmo prestador recebe o mesmo pedido a
--      cada clique duplo, cada retry e cada disparo simultâneo. Índice único
--      (demanda_id, usuario_id); a gravação ignora duplicado e só manda push
--      pra quem VOLTOU da gravação — quem já tinha sido avisado não volta.
--      Leitura só do próprio, e NENHUMA policy de escrita: só a chave de
--      serviço grava. Traz junto a função que responde ao autor QUANTOS foram
--      avisados — inteiro, nunca linha: a lista de prestadores de uma região é
--      o ativo da plataforma, e publicar um pedido não é credencial pra
--      baixá-la.
--
-- 090  `conversas`, `mensagens` e `conversas_leitura` — a conversa entre as
--      duas partes de UMA proposta. Não é uma sala por pedido, e a diferença é
--      a razão de existir do desenho: um pedido com três respostas vira TRÊS
--      conversas, senão três concorrentes leem o preço um do outro. Isso é
--      vazamento por modelagem, e policy nenhuma conserta depois.
--
--      Três travas que moram no banco e não na tela:
--        · nem admin lê — e a migration ABORTA se alguma policy do módulo
--          citar `eh_admin()`, porque colar isso numa migration futura
--          passaria despercebido num diff grande;
--        · ninguém apaga a sala com o registro do outro dentro: não há policy
--          de UPDATE nem de DELETE na conversa, e as chaves são `restrict` e
--          não `cascade` — com cascade, o autor do pedido teria um botão
--          indireto pra destruir a negociação alheia;
--        · editar mensagem não existe: um gatilho converte UPDATE em apagar,
--          carimbando a hora decidida pelo BANCO, esvaziando o corpo e
--          mantendo autor e horário. Apagar duas vezes levanta exceção.
--
-- ---------------------------------------------------------------------------
-- IMPACTO NO ACESSO: ninguém ganha nem perde permissão sobre o que já existe.
-- As tabelas nascem vazias e com RLS ligada.
--
-- REALTIME: as tabelas NÃO foram adicionadas à publication, de propósito. Ver
-- o motivo no cabeçalho da 090.
--
-- SE ALGO DER ERRADO: a reversão de cada uma está no fim do próprio arquivo de
-- migration, em comentário.
-- ===========================================================================
...
-- ###########################################################################
-- ### 090_conversa_entre_as_duas_partes.sql
-- ###########################################################################
...
-- ===========================================================================
-- DEPOIS DE RODAR: as duas conferências finais estão no fim de cada bloco
-- acima. Se as duas passarem, o app pode subir.
-- ===========================================================================
