-- ===========================================================================
-- COMMANDER — SÉTIMO LOTE, 19/08/2026
-- A DISTÂNCIA DA TRILHA GRAVADA UMA VEZ
--
-- ***** ESTE LOTE TRAVA O DEPLOY. RODE ANTES DE EU SUBIR O APP. *****
--
-- Não é teoria: foi testado. Sem a coluna, `/diario` mostra "Não foi possível
-- carregar o diário" e a Início perde o cartão do ano em silêncio.
--
...
-- O QUE ELA CONSERTA
-- ---------------------------------------------------------------------------
-- A tela inicial baixava a TRILHA GPS INTEIRA de todas as saídas do ano só
-- para calcular uma distância em milhas. Medido no banco: uma trilha cheia tem
-- 205,6 kB. Um barco que sai três vezes por semana chega a dezembro com 144
-- saídas — 28,9 MB por abertura da Início, e o Diário com 300 saídas desce
-- 60,3 MB de traçado. A conta a 10.000 barcos dava US$ 1.417/mês só de banda.
--
-- O conserto já estava meio feito e ninguém tinha percebido: `salvarTrilha` e
-- `importarTrilhas` JÁ CALCULAVAM o resumo na hora de gravar — e jogavam o
-- número fora. Agora ele é gravado. A linha da consulta do ano cai de 210.687
-- para 124 bytes; aos 144 saídas, 28,9 MB viram 17,4 kB. A trilha crua só desce
-- quando alguém abre a saída no mapa, que é quando ela serve para alguma coisa.
--
-- ---------------------------------------------------------------------------
-- O BACKFILL, E A CONFERÊNCIA QUE ELE JÁ PASSOU
-- ---------------------------------------------------------------------------
-- As linhas que já existem são preenchidas pela própria migration, com a
-- fórmula de haversine escrita em SQL. Ela foi conferida contra a função que o
-- app usa (`resumoTrilha`) sobre a MESMA trilha de 4.000 pontos: 35,038275 nm
-- nos dois, diferença de 4,5 × 10⁻⁷.
--
-- E saída SEM GPS continua sem distância — `null`, nunca "0 MN". É a regra da
-- casa (`lib/domain/patio.ts`): zero desenhado quer dizer "não teve", e uma
-- saída sem trilha teve distância, só não foi medida. Há teste.
--
-- ---------------------------------------------------------------------------
-- IMPACTO NO ACESSO: ZERO. Nenhuma policy muda, ninguém ganha nem perde
-- permissão. É uma coluna nova e o preenchimento dela.
--
-- SE ALGO DER ERRADO: a reversão está no fim do próprio arquivo, em comentário.
-- ===========================================================================
...
-- ###########################################################################
-- ### 092_distancia_da_trilha_gravada_uma_vez.sql
-- ###########################################################################
