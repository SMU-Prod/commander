...
export function SeletorEmbarcacao({
  ...
  return (
    <span className="relative inline-block">
      ...
      {aberto && (
        <span className="sombra-2 absolute left-0 top-11 z-20 min-w-[200px] rounded-[var(--raio-cartao)] border border-line bg-panel p-1">
          {opcoes.map((o) => (
            <form key={o.id} action={definirEmbarcacaoAtiva}>
              <input type="hidden" name="embarcacao_id" value={o.id} />
              <button className={`flex h-11 w-full items-center rounded-[var(--raio-controle)] px-3 corpo ${
          ...
          <a
            href="/onboarding"
            className="corpo flex h-11 w-full items-center gap-2 rounded-[var(--raio-controle)] px-3 text-accent-forte"
          >
