              ...
              <div key={c.id} className={`sombra-1 rounded-[var(--raio-cartao)] border p-4 ${c.duplicada ? "border-line bg-panel opacity-70" : "border-line bg-panel"}`}>
                <label className="flex min-h-11 cursor-pointer items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {c.duplicada && (
                      <p className="mt-1 inline-flex items-center gap-1 rounded-[var(--raio-pilula)] border border-line px-2 py-0.5 tabular-nums text-xs text-dim">
                        Já importada antes — será pulada
                      </p>
                    )}
                    {contribuirCorredor ? (
                      <p className="mt-1 tabular-nums text-xs text-accent-forte">Vai pro diário e também vira passagens no mapa de corredores</p>
                    ) : (
                      <p className="mt-1 tabular-nums text-xs text-dim">Vai só para o diário</p>
                    )}
                  </div>
                </label>
