      ...
      <div className="mt-4 flex gap-1.5">
        {([["mensal", "Mensal"], ["anual", "Anual"], ["livre", "Período"]] as const).map(([valor, rotulo]) => (
          <Link
            ...
            /* `h-11` e não `h-10`: 40px é 4px abaixo da régua de toque do app
               (DESIGN §5, "44px sem exceção") e esta é a fila que se toca a
               cada troca de período. É a MESMA altura do `Chip`, que é a
               única altura de pílula do app. */
            className={`flex h-11 flex-1 items-center justify-center rounded-[var(--raio-pilula)] border text-sm font-medium ${
      ...
      {modo === "mensal" && (
        <form className="mt-3 flex gap-2">
          ...
          <select name="mes" defaultValue={mesSelecionado}
            className="h-11 flex-1 rounded-[var(--raio-controle)] border border-line bg-campo px-3 text-base">
            {meses.map((m) => (
              <option key={m} value={m}>{periodoMensal(anoSelecionado, m).rotulo.split(" de ")[0]}</option>
            ))}
          </select>
          <select name="ano" defaultValue={anoSelecionado}
            className="h-11 w-28 rounded-[var(--raio-controle)] border border-line bg-campo px-3 text-base">
            {anos.map((a) => <option key={a} value={a}>{a}</option>)}
          </select>
          <button className="h-11 rounded-[var(--raio-controle)] border border-line px-4 text-sm font-semibold">Ver</button>
        </form>
      )}
