...
// ONDA 99 (P2-5) — a forma da linha vem de `lib/db/types.ts`, derivada do
// banco vivo. A cópia que morava aqui declarava 8 das 15 colunas, e a mais
// grave das ausentes era `embarcacao_id`: esta tela filtra por ele em toda
// consulta e o tipo fingia que a coluna não existia.
//
// A nota do A15 sobre `entrada_em` foi junto para o tipo promovido — "Entrada
// na oficina" era pedida no formulário, gravada por `abrirServico` e ausente
// deste tipo, então a data que responde "há quanto tempo esse motor está
// parado lá" ficava só no banco. Quem a transforma em resposta é
// `tempoNaOficina`: a coluna é uma data, mas a pergunta é um intervalo.

/*
 * A15, A METADE QUE FICA EM ABERTO DE PROPÓSITO — E QUEM LER ISTO NÃO ESTÁ
 * OLHANDO PARA DADO PERDIDO.
 *
 * `servicos_mecanica.anexo_path` e `orcamentos.anexo_path` existem desde a
 * migration 063 (linhas 64 e 91) e estão VAZIAS EM 100% DAS LINHAS, porque
 * nenhuma tela pede o arquivo e nenhuma action o grava: não há caminho de
 * escrita, em lugar nenhum do app. Isso é diferente do resto do A15 — ali o
 * dado era coletado e engolido; aqui ele nunca chegou a existir.
 *
 * NÃO É PARA "CORRIGIR" ISTO MOSTRANDO A COLUNA. Renderizar um campo que
 * ninguém preenche não devolve anexo nenhum a ninguém; devolve um rótulo
 * "Anexo: —" em toda linha da tela. O que fecha de verdade é o upload
 * completo — seletor de arquivo, validação de MIME, bucket `acervo`, URL
 * assinada na leitura —, que é o padrão já escrito em
 * `lib/acoes/ocorrencias.ts` e `app/(app)/barco/equipamento/[id]`. É trabalho
 * de onda própria, e foi deixado FORA desta por escolha declarada, não por
 * esquecimento.
 *
 * O caso do orçamento é o que mais dói e é o que justifica a nota: o
 * fornecedor manda o orçamento em PDF, e hoje ele vive no WhatsApp de alguém.
 * É o mesmo defeito que a auditoria de 08/08 já corrigiu no Diário.
 */
...
export default async function MecanicaPage({
  ...
  // ONDA 99 (P2-5) — `Orcamento` e `Votacao` também saem de `types.ts` agora.
  // A votação continua declarada AQUI, e de propósito: ela não é a linha da
  // tabela, é a linha MAIS o join `votos(voto)` que a consulta acima pede.
  // Isso é projeção, não duplicata — e a diferença importa, porque promover
  // uma projeção pra `types.ts` faria o arquivo do banco prometer uma coluna
      ...
      {editavel && (
        <>
          ...
          {/* `--raio-cartao` e não `--raio-painel`: os 14px cravados aqui e nos
              outros dois painéis desta tela eram o mesmo desenho dos cartões
              de orçamento e dos painéis da bancada, que já vinham por token.
              Promover só o que estava à mão deixaria dois raios no mesmo nível
              da mesma tela. Subir a tela inteira está no relatório. */}
          <form action={abrirServico} className={`sombra-1 space-y-3 rounded-[var(--raio-cartao)] border border-line bg-panel p-4 ${TETO_FORMULARIO}`}>
            ...
            {/* Era `rounded-xl` — 12px, degrau que a escala não tem. Botão se
                ...
                igual pro "Registrar orçamento" mais abaixo. */}
...
function LinhaServico({
  ...
  /** A data de hoje vem de cima (`hojeISO()`) e não de `new Date()` aqui: é o
   *  mesmo dia civil que o resto da tela usa, e um Server Component que lê o
   *  relógio por conta própria daria dois "hoje" na mesma página. */
  hoje: string
}) {
  const tom = tomDoServico(s.estado)
  // A15 — o tempo de bancada entra TAMBÉM na linha compacta do desktop, e é
  // aqui que ele mais paga: esta é a lista que o ADM varre atrás de "qual
  // unidade está parada há mais tempo", e ler vinte datas para subtrair de
  // cabeça é exatamente o trabalho que a coluna existia pra poupar.
...
function CartaoServico({
  ...
  return (
    ...
    >
      ...
      <p className="apoio mt-1 text-dim">
        {/* A15 — a entrada na oficina, devolvida a quem a digitou COMO
            INTERVALO. A primeira correção deste achado escrevia a data crua
            ("entrou em 05/08"), o que fecha o achado pela letra e não pelo
            espírito: o que essa coluna existe pra responder é "há quanto tempo
            esse barco está parado lá", e uma data solta obriga a subtração de
            cabeça em toda linha. `tempoNaOficina` decide a frase — inclusive a
            de não escrever nada quando ninguém anotou a data, que é o caso da
